...
def solve_samurai(fname, n_cols_rows, values):

    # Prepare the model and  the flags
    model, flags = init_model(f'Samurai {fname} Model', n_cols_rows, values, num_cell_constraints)
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print(f'Usage: ./{this} filename.samurai')
        quit()

    ncr = 21
    vals = list(range(1, 10))
    solve_samurai(sys.argv[1], ncr, vals)
